CREATE OR REPLACE FUNCTION ${schema}.extrato_functionBB("pEXT_CODIGO" bigint, "pAG_CODIGO" bigint)
 RETURNS numeric
 LANGUAGE plpgsql
AS $function$
DECLARE
...
SELECT  a.ID, A.CONTA_ID INTO vAGENCIAAPLICACAOID, vCONTAAPLICACAOID from ${schema}.AGENCIABANCARIA a where a.PARCEIRO_ID = vPARCEIROID AND a.TIPO_AGENCIA = 'APLICACAO' AND a.AGE_AGENCIA = vAGEAGENCIA AND a.AGE_NUMERO = vAGENUMERO;
  ...
  vAPLICACAO_AUT := 0;
  IF (vCREDITO IS NOT null and (vNUMERODOCUMENTO = '2' OR vNUMERODOCUMENTO = '5' OR vNUMERODOCUMENTO = '42') AND vAGENCIABANCARIAID = pAG_CODIGO and vAGENCIAAPLICACAOID is not null) THEN
	...
	vAPLICACAO_AUT := 1;
END IF;	
IF (vDEBITO IS NOT null and (vNUMERODOCUMENTO = '2' OR vNUMERODOCUMENTO = '5' OR vNUMERODOCUMENTO = '42') AND vAGENCIABANCARIAID = pAG_CODIGO and vAGENCIAAPLICACAOID is not null) THEN
