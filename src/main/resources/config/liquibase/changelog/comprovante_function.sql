...
BEGIN
  ...
  EXIT WHEN NOT FOUND;
   ...
   IF (vDETALHECOMP = vNUMERODOCUMENTO) AND (vDATAPAGAMENTO = vDATAEXTRATO) AND (vVALORPAGAMENTO = vDEBITO*-1 ) THEN
     UPDATE  ds_demo.INTELIGENT SET COMPROVANTE_ID= vCOMPROVANTE,CNPJ= vCNPJ,  BENEFICIARIO= vBENEFICIARIO, TIPO_INTELIGENT = vTIPOINTELIGENTE, TIPO_VALOR = 'PRINCIPAL' WHERE ID = vCODIGOINTELIGENTE;
     IF ((vCOMMULTA > 0) OR (vCOMJUROS >0)) THEN
     ...
     END IF;
     IF (VCOMDESCONTO > 0) THEN
     	INSERT INTO  ds_demo.INTELIGENT (historico, historicofinal,tipo_valor,datalancamento,numerodocumento,numerocontrole,periodo,credito,associado,
