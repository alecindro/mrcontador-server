CREATE OR REPLACE FUNCTION ds_demo.extrato_teste("pEXT_CODIGO" bigint, "pAG_CODIGO" bigint)
 RETURNS numeric
 LANGUAGE plpgsql
AS $function$
DECLARE
  ...
  SELECT  DISTINCT ID, EXT_DATALANCAMENTO, EXT_HISTORICO, EXT_NUMERODOCUMENTO, EXT_NUMEROCONTROLE, EXT_DESCRICAO, EXT_DEBITO, EXT_CREDITO, 
  AGENCIABANCARIA_ID, PARCEIRO_ID
...
cEXTRATO CURSOR for SELECT  ID, COM_CNPJ, COM_BENEFICIARIO, 
    COM_DATAVENCIMENTO, 
    COM_VALORDOCUMENTO, 
    COM_VALORPAGAMENTO, 
    COM_DATAPAGAMENTO,     
	...
	WHERE PARCEIRO_ID= vPARCEIROID
      AND AGENCIABANCARIA_ID= vAGENCIABANCARIAID
      AND processado = FALSE
      and COM_DOCUMENTO = vNUMERODOCUMENTO
      and COM_DATAPAGAMENTO = vDATAEXTRATO
      and COM_VALORPAGAMENTO = (vDEBITO*-1)
    ORDER BY e.ID 
    LIMIT 1	;
 vBANCOID numeric;
vAGEAGENCIA text;
...
BEGIN 
 ...
 SELECT   a.PARCEIRO_ID, a.BANCO_ID, a.AGE_AGENCIA, a.AGE_NUMERO INTO vPARCEIROID, vBANCOID, vAGEAGENCIA, vAGENUMERO from ds_demo.AGENCIABANCARIA a where a.ID = pAG_CODIGO AND a.POSSUEAPLICACAO = TRUE;
SELECT  a.ID, A.CONTA_ID INTO vAGENCIAAPLICACAOID, vCONTAAPLICACAOID from ds_demo.AGENCIABANCARIA a where a.PARCEIRO_ID = vPARCEIROID AND a.TIPO_AGENCIA = 'APLICACAO' AND a.AGE_AGENCIA = vAGEAGENCIA AND a.AGE_NUMERO = vAGENUMERO;
  ...
  EXIT WHEN NOT FOUND;
   ...
   UPDATE ds_demo.EXTRATO SET processado = true where id = pEXT_CODIGO;
  
  
  vAPLICACAO_AUT := 0;
  IF (vCREDITO IS NOT null and vNUMERODOCUMENTO = '2' AND vAGENCIABANCARIAID = pAG_CODIGO) THEN
	vHISTORICOFINAL:='Crédito ref. resgate de Aplicação Automática';
	UPDATE ds_demo.INTELIGENT SET ASSOCIADO = TRUE, HISTORICOFINAL = vHISTORICOFINAL, TIPO_INTELIGENT = 'x', CONTA_ID = vCONTAAPLICACAOID WHERE extrato_id = vCODIGEXTRATO;
	INSERT INTO ds_demo.INTELIGENT (ASSOCIADO,DATALANCAMENTO,HISTORICOFINAL,PERIODO,DEBITO,TIPO_INTELIGENT, PARCEIRO_ID,AGENCIABANCARIA_ID,EXTRATO_ID, CONTA_ID) 
	...
	vAPLICACAO_AUT := 1;
END IF;	
IF (vDEBITO IS NOT NULL) THEN
	vHISTORICOFINAL:='Débito ref. operação de aplicação Automática';
	UPDATE ds_demo.INTELIGENT SET ASSOCIADO = TRUE, HISTORICOFINAL = vHISTORICOFINAL, TIPO_INTELIGENT = 'x', CONTA_ID = vCONTAAPLICACAOID WHERE extrato_id = vCODIGEXTRATO;
	INSERT INTO ds_demo.INTELIGENT (ASSOCIADO,DATALANCAMENTO,HISTORICOFINAL,PERIODO,CREDITO,TIPO_INTELIGENT, PARCEIRO_ID,AGENCIABANCARIA_ID,EXTRATO_ID, CONTA_ID) 
...
if (vAPLICACAO_AUT > 0) then
   for RECEXTRATO in select C.ID, C.COM_DOCUMENTO, C.COM_CNPJ, C.COM_BENEFICIARIO, C.COM_DATAVENCIMENTO, C.COM_VALORDOCUMENTO, C.agenciabancaria_id,
	C.COM_VALORPAGAMENTO, C.COM_DATAPAGAMENTO, C.PARCEIRO_ID, C.AGENCIABANCARIA_ID, C.COM_MULTA,
     C.COM_JUROS, C.COM_DESCONTO 
    FROM  ds_demo.COMPROVANTE C
   ...
   UPDATE  ds_demo.INTELIGENT SET COMPROVANTE_ID= RECEXTRATO.ID,CNPJ= RECEXTRATO.COM_CNPJ,  BENEFICIARIO= RECEXTRATO.COM_BENEFICIARIO, TIPO_INTELIGENT = vTIPOINTELIGENTE, TIPO_VALOR = 'PRINCIPAL', HISTORICOFINAL = vHISTORICOFINAL WHERE extrato_id = vCODIGEXTRATO;
       IF ((RECEXTRATO.COM_MULTA > 0) OR (RECEXTRATO.COM_JUROS >0)) THEN
     	vHISTORICOFINAL   := 'Pagto. de Juros de '|| RECEXTRATO.COM_BENEFICIARIO;
        UPDATE  ds_demo.INTELIGENT SET debito = RECEXTRATO.COM_VALORDOCUMENTO*-1 WHERE extrato_id = vCODIGEXTRATO;
      ...
      UPDATE  ds_demo.INTELIGENT SET debito = RECEXTRATO.COM_VALORDOCUMENTO*-1 WHERE extrato_id = vCODIGEXTRATO;
     	INSERT INTO  ds_demo.INTELIGENT (historico, tipo_valor,datalancamento,numerodocumento,numerocontrole,periodo,credito,associado,
			...
			('Receb. de desconto','DESCONTO',vDATAEXTRATO,vNUMERODOCUMENTO,vNUMEROCONTROLE,vPERIODO, RECEXTRATO.COM_DESCONTO,false,
     		RECEXTRATO.COM_CNPJ,RECEXTRATO.COM_BENEFICIARIO, vTIPOINTELIGENTE,RECEXTRATO.ID,vPARCEIROID,vAGENCIABANCARIAID, vCODIGEXTRATO, vHISTORICOFINAL);
     END IF;
  UPDATE ds_demo.COMPROVANTE SET PROCESSADO = TRUE WHERE ID = RECEXTRATO.ID;
 end loop;
END IF;
  ...
  IF (vRETORNO > 0) then
  RAISE NOTICE 'Calling nota fiscal (%)',vPARCEIROID;
  SELECT ds_demo.processa_notafiscalgeral(CAST(vPARCEIROID AS int8)) INTO vQUANTIDADENOTA;
  END IF;
