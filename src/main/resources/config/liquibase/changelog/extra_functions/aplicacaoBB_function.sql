CREATE OR REPLACE FUNCTION ${schema}.aplicacaoBB_function("pAG_CODIGO" bigint)
 RETURNS numeric
 LANGUAGE plpgsql
AS $function$
DECLARE
...
AND I.ASSOCIADO = FALSE AND (E.EXT_NUMERODOCUMENTO = '2' OR E.EXT_NUMERODOCUMENTO = '5' OR E.EXT_NUMERODOCUMENTO = '42') AND I.AGENCIABANCARIA_ID = pAG_CODIGO

LOOP
IF (REC.CREDITO IS NOT NULL) THEN
	vHISTORICOFINAL:='Crédito ref. resgate de Aplicação Automática - Banco do Brasil';
...
IF (REC.DEBITO IS NOT NULL) THEN
	vHISTORICOFINAL:='Débito ref. operação de aplicação Automática - Banco do Brasil';
	UPDATE ${schema}.INTELIGENT SET ASSOCIADO = TRUE, HISTORICOFINAL = vHISTORICOFINAL, TIPO_INTELIGENT = 'x', CONTA_ID = vCONTAAPLICACAOID WHERE ID = REC.INTELIGENT_ID;
