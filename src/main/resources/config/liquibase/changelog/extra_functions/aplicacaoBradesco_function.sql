CREATE OR REPLACE FUNCTION ${schema}.aplicacaoBradesco_function("pAG_CODIGO" bigint)
 RETURNS numeric
 LANGUAGE plpgsql
AS $function$
DECLARE
...
AND I.ASSOCIADO = FALSE AND (E.ext_historico like '%APLIC. INVEST FACIL%') AND I.AGENCIABANCARIA_ID = pAG_CODIGO

LOOP
IF (REC.CREDITO IS NOT NULL) THEN
	vHISTORICOFINAL:='Crédito ref. Aplicação Automática - Bradesco';
...
IF (REC.DEBITO IS NOT NULL) THEN
	vHISTORICOFINAL:='Débito ref. Aplicação Automática - Bradesco';
	UPDATE ${schema}.INTELIGENT SET ASSOCIADO = TRUE, HISTORICOFINAL = vHISTORICOFINAL, TIPO_INTELIGENT = 'x', CONTA_ID = vCONTAAPLICACAOID WHERE ID = REC.INTELIGENT_ID;
