 ...
 LOOP
    ...
    IF (vTAXA > 0) THEN
	   	...
	   	INSERT INTO  ds_demo.INTELIGENT (historico, historicofinal,tipo_valor,datalancamento,numerodocumento,numerocontrole,periodo,debito,associado,
	     		cnpj,beneficiario,tipo_inteligent,comprovante_id,parceiro_id,agenciabancaria_id, extrato_id) VALUES ('Pagto. de Taxa bancária','Pagto. de Taxa bancária','TAXA',REC.vDATAEXTRATO,REC.vNUMERODOCUMENTO,
	     		REC.vNUMEROCONTROLE,REC.vPERIODO, vTAXA,false,
	...
	vRETORNO:= vRETORNO + vRETORNOCONTA;
 END LOOP; 
	if (vRETORNO > 0) THEN
