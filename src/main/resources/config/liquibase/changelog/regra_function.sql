...
vRETORNO := 0;	
	...
	LOOP		
	  FOR REC_INTELIGENT IN
		SELECT i.* FROM ${schema}.INTELIGENT i inner join extrato e on i.extrato_id = e.id WHERE i.PARCEIRO_ID = pParceiroId AND i.PERIODO = pPeriodo and i.associado = false AND ((i.historico = REC_REGRA.REG_DESCRICAO AND REC_REGRA.TIPO_REGRA = 'HISTORICO')
