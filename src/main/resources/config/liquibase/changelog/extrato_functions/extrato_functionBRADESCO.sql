CREATE OR REPLACE FUNCTION ${schema}.extrato_functionBRADESCO("parceiroId" bigint, "agenciaID" bigint, "pPeriodo" varchar)
 RETURNS numeric
 LANGUAGE plpgsql
AS $function$
DECLARE
...
where a.PARCEIRO_ID = pParceiroId AND a.TIPO_AGENCIA = 'APLICACAO' AND a.AGE_AGENCIA = vAGEAGENCIA
 ...
 -- Aplicacao automatica
  vAPLICACAO_AUT:=0;
    IF (REGEXP_REPLACE(REC_EXTRATO.ext_historico, '\s', '', 'g') like '%APLIC.INVESTFACIL%' AND REC_EXTRATO.agenciabancaria_id = pAgenciaId
     and vAGENCIAAPLICACAOID is not null) THEN
	...
	vAPLICACAO_AUT := 1;
ELSEIF (REGEXP_REPLACE(REC_EXTRATO.ext_historico, '\s', '', 'g') like '%RESGATEINVESTFACIL%' AND REC_EXTRATO.agenciabancaria_id = pAgenciaId and vAGENCIAAPLICACAOID is not null) THEN
