...
SELECT  a.ID, A.CONTA_ID INTO vAGENCIAAPLICACAOID, vCONTAAPLICACAOID from ${schema}.AGENCIABANCARIA a where a.PARCEIRO_ID = vPARCEIROID AND a.TIPO_AGENCIA = 'APLICACAO' AND a.AGE_AGENCIA = vAGEAGENCIA AND a.AGE_NUMERO = vAGENUMERO;
  ...
  vAPLICACAO_AUT := 0;
  IF (REGEXP_REPLACE(vHISTORICO, '\s', '', 'g') like '%APLIC.INVESTFACIL%' AND vAGENCIABANCARIAID = pAG_CODIGO and vAGENCIAAPLICACAOID is not null) THEN
	...
	vAPLICACAO_AUT := 1;
END IF;	
IF (REGEXP_REPLACE(vHISTORICO, '\s', '', 'g') like '%RESGATEINVESTFACIL%' AND vAGENCIABANCARIAID = pAG_CODIGO and vAGENCIAAPLICACAOID is not null) THEN
