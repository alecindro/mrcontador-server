CREATE OR REPLACE FUNCTION ${schema}.extrato_functionSICRED("parceiroId" bigint, "agenciaID" bigint, "pPeriodo" character varying)
 RETURNS numeric
 LANGUAGE plpgsql
AS $function$
DECLARE
...
END IF; 	
 END LOOP;
   IF (vRETORNO > 0) THEN 
        SELECT * INTO vRETORNO_COMPROVANTE FROM ${schema}.comprovante_sicred(pParceiroId,pAgenciaId,pPeriodo); 
