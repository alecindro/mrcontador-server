CREATE OR REPLACE FUNCTION ${schema}.extrato_functionCREDICREA("parceiroId" bigint, "agenciaID" bigint, "pPeriodo" character varying)
 RETURNS numeric
 LANGUAGE plpgsql
AS $function$
DECLARE
...
where a.PARCEIRO_ID = pParceiroId AND a.TIPO_AGENCIA = 'APLICACAO' AND a.AGE_AGENCIA = vAGEAGENCIA
 ...
 END LOOP;
  IF (vRETORNO > 0) THEN 
        SELECT * INTO vRETORNO_COMPROVANTE FROM ${schema}.comprovante_credicrea(pParceiroId,pAgenciaId,pPeriodo); 
