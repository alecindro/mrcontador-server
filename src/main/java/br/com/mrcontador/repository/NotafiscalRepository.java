...
public interface NotafiscalRepository extends JpaRepository<Notafiscal, Long>, JpaSpecificationExecutor<Notafiscal> {
	...
	void updateArquivo(@Param("notaId") Long id, @Param("arquivoId") Long arquivoId,
			@Param("arquivoPdfId") Long arquivoPdfId);
	...
	List<Notafiscal> find(@Param("cnpj") String cnpj, @Param("valorInicial") BigDecimal valorInicial,
			@Param("valorFinal") BigDecimal valorFinal, @Param("datainicial") LocalDate datainicial,
	...
	@Query(value = "select COUNT(id) as total from notafiscal where parceiro_id = :parceiroId", nativeQuery = true)
	Optional<Long> getTotal(@Param("parceiroId") Long parceiroId);
