...
public class ComprovanteBB extends ComprovanteBanco{
	...
	private List<Comprovante> parseTransfPoupanca(String[] lines, Agenciabancaria agenciabancaria, Parceiro parceiro) throws ComprovanteException{
		List<DiffValue> list = new ArrayList<DiffValue>();
		int i = 0;
		for (String line : lines) {
			line = StringUtils.normalizeSpace(line.trim());
	...
	public void callFunction(List<Comprovante> comprovantes, ComprovanteService service) {
		comprovantes.forEach(comprovante ->{
			service.callComprovanteBB(comprovante.getId());	
		});
